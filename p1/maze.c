...
Maze *maze_addPoint(Maze *pmaze, const Point *ppoint){
	...
	if(point_getSymbol(ppoint) == ERRORCHAR||(point_getCoordinateX(ppoint)) < 0||(point_getCoordinateY(ppoint)) < 0){
		return NULL;
	}
...
Point * maze_getNeighborPoint(const Maze *pmaze, const Point *ppoint, const Move mov){
	...
	if(mov == RIGHT){
		if(point_getCoordinateX(ppoint) == ((pmaze->ncol)-1)){
			return NULL;
		}
	...
	if(mov == DOWN){
		if(point_getCoordinateY(ppoint) == ((pmaze->nrow) - 1)){
		return pmaze->ppoint[(point_getCoordinateY(ppoint))+1][point_getCoordinateX(ppoint)];
		}
	}
	if(mov == STAY){
