 ...
 *
 * Created on 18 de abril de 2016, 23:37
 */
...
Status tree_insertREC(NodeBT **pnodo, const EleTree* po){
	NodeBT *nodo = NULL;
	EleTree *pele= NULL;
...
Bool tree_findEleTreeREC(NodeBT *nodo, const EleTree *pele){
	int cmp;
	if(!nodo){
		return FALSE;
	}
	cmp = eletree_cmp(nodo->info, (EleTree *)pele);
	if(cmp == 0){
		return TRUE;
	}
	else if(cmp < 0){ /* Busco dependiendo del valor del emento para agilizar la búsqueda */
		return tree_findEleTreeREC(nodo->right, pele);
	}
	else{
		return tree_findEleTreeREC(nodo->left, pele);
	}
}
...
Status tree_inOrderREC(FILE *f, NodeBT* nodo){
	...
	if(tree_inOrderREC(f, nodo->left) == ERROR) return ERROR;
	if(eletree_print(f, nodo->info) < 0) return ERROR;
	if(tree_inOrderREC(f, nodo->right) == ERROR) return ERROR;
	return OK;
}
...
Status tree_postOrderREC(FILE *f, NodeBT* nodo){
	...
	if(tree_postOrderREC(f, nodo->left) == ERROR) return ERROR;
	if(tree_postOrderREC(f, nodo->right) == ERROR) return ERROR;
	if(eletree_print(f, nodo->info) < 0) return ERROR;
	return OK;
}
...
Status tree_insert(Tree *pa, const EleTree *po){
	...
	return tree_insertREC(&(pa->root), po);
}
...
Bool tree_findEleTree(Tree* pa, const EleTree* pe){
	...
	return tree_findEleTreeREC(pa->root, pe);
}
